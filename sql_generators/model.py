...
station_measured_props = Table('StationMeasuredProps', Base.metadata,
    Column('station_id', Unicode(10), ForeignKey('Stations.code'), primary_key=True),
    Column('prop_id', Unicode(50), ForeignKey('Properties.name'), primary_key=True)
    )
...
class Observation(Base):
	...
	station_id = Column(Unicode(10), ForeignKey('Stations.code'), nullable = False)
	station = relation(Station.__name__, backref = backref('stations', order_by=id, cascade = 'all,delete'))

	prop_id = Column(Unicode(50), ForeignKey('Properties.name'), nullable = False)
